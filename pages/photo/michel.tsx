...
import { michel } from "../../components/Work/images";
import projectData from "../../components/Work/projectData";

function Michel() {
  const project = projectData.find(
    (project: any) => project.name === "Michel Freudenberg"
  );
              ...
              <div className="w-full h-min flex items-start justify-between ">
                ...
                <div className="w-1/2 h-full">
                  <p className="text-subtitle text-dark">
                    ...
                    consectetur, adipisicing elit. Numquam officiis impedit eos
                    assumenda repellat esse quia magnam quos praesentium, earum
                    architecto provident nesciunt eum amet quibusdam explicabo!
                    Nobis, perferendis vitae!
                  </p>
                </div>
              </div>
            </div>
                ...
                {merged.map((arr, index) => {
                  if (arr.length === 2) {
                    return (
                      ...
                      >
                        <div
                          className={`h-full [aspect-ratio:${arr[0].width}/${arr[0].height}] relative`}
                          key={index}
                        >
                          <Image
                            src={arr[0]}
